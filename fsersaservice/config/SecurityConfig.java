 ...
 *   /v1/workflow/status          — callback Gateway FSE 2.0 (il Gateway non invia API key)
 *
 * PROTETTI con X-API-Key:
 *   /api/**                      — tutti gli endpoint applicativi
 *
 * NEGATI (403) per tutto il resto:
 *   /actuator/metrics, /actuator/env, ecc.
...
public class SecurityConfig {
    ...
    private static class ApiKeyWebFilter implements WebFilter {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
            ...
            // 2. Callback Gateway FSE 2.0: aperto — il Gateway non invia API key.
            //    Sicurezza aggiuntiva: il traffico è limitato alla rete interna
            //    (il Gateway Docker gira in localhost).
            if (isGatewayCallbackPath(path)) {
                return chain.filter(exchange);
            }

            // 3. Endpoint applicativi: richiedono API key
            ...
            // 4. Tutto il resto (altri /actuator/**, path sconosciuti): 403
        ...
        /**
         * Path di callback del Gateway FSE 2.0.
         * Il Gateway chiama questo endpoint dopo le operazioni asincrone
         * (pubblicazione su INI/EDS/UA-R) senza API key.
         */
        private boolean isGatewayCallbackPath(String path) {
            return path.equals("/v1/workflow/status")
                    || path.startsWith("/v1/workflow/status/");
        }
