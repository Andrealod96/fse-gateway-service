...
public class WebClientConfig {
    ...
    private SslContext buildGatewaySslContext() {
        try {
            ...
            X509ExtendedKeyManager keyManager =
                    extractX509KeyManager(keyManagerFactory.getKeyManagers());

            return SslContextBuilder.forClient()
                    .keyManager(new ForcedAliasX509ExtendedKeyManager(
                            keyManager,
                            mtlsProperties.getKeyAlias()
                    ))
            ...
            throw new IllegalStateException(
                    "Errore nella configurazione SSL/mTLS del WebClient Gateway",
                    ex
            );
        }
    }
    ...
    private X509ExtendedKeyManager extractX509KeyManager(KeyManager[] keyManagers) {
        return Arrays.stream(keyManagers)
                ...
                .findFirst()
                .orElseThrow(() ->
                        new IllegalStateException("Nessun X509ExtendedKeyManager trovato nel keystore mTLS")
                );
    }
}