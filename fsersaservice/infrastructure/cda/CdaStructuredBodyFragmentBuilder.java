...
public class CdaStructuredBodyFragmentBuilder {
    ...
    public String build(RsaDocument document, String documentEffectiveTime) {
        ...
        appendStoriaClinica(xml, document, documentEffectiveTime);
    ...
    private void appendQuesitoDiagnostico(StringBuilder xml, RsaDocument document) {
        ...
        boolean hasNarrativeText = hasText(quesito.getText());
        boolean hasCoding = hasAnyCodingValue(quesito.getCoding());

        if (!hasNarrativeText && !hasCoding) {
            return;
        }
    ...
    private void appendStoriaClinica(StringBuilder xml, RsaDocument document, String documentEffectiveTime) {
        ...
        if (hasStructured) {
            ...
            appendAllergieSubSection(xml, storia.getAllergies(), document, documentEffectiveTime);
    ...
    private void appendFamilyHistoryEntries(StringBuilder xml, List<RsaDocument.FamilyHistory> familyHistories) {
        ...
        for (RsaDocument.FamilyHistory fh : familyHistories) {
            ...
            /*
             * Regola importante per i KO:
             *
             * - relationshipCode valorizzato:
             *   genera relatedSubject/code con gli attributi realmente presenti.
             *
             * - relationshipCode assente:
             *   NON genera relatedSubject/code.
             *   Serve per il KO sul grado di parentela non presente.
             *
             * Non generare mai <code code="" displayName=""/>: quello produce
             * un errore sintattico su attributi vuoti, diverso dal KO richiesto.
             */
            if (hasText(fh.getRelationshipCode())) {
                xml.append("<code");
                appendAttributeIfPresent(xml, "code", fh.getRelationshipCode());
                appendAttributeIfPresent(xml, "codeSystem", defaultIfBlank(fh.getRelationshipCodeSystem(), "2.16.840.1.113883.5.111"));
                appendAttributeIfPresent(xml, "codeSystemName", defaultIfBlank(fh.getRelationshipCodeSystemName(), "RoleCode"));
                appendAttributeIfPresent(xml, "displayName", fh.getRelationshipDisplayName());
                xml.append("/>");
            }

            xml.append("<subject>");
            appendAdministrativeGenderCode(xml, fh.getSubjectGenderCode());
            xml.append("</subject>");
    ...
    private void appendAllergieSubSection(
            StringBuilder xml,
            List<RsaDocument.Allergy> allergies,
            RsaDocument document,
            String documentEffectiveTime
    ) {
        ...
        for (RsaDocument.Allergy allergy : valid) {
            String allergyId = "ALL-" + narrativeIndex;

            xml.append("<item>");
            xml.append("<content ID=\"").append(allergyId).append("\">")
                    .append(t(defaultIfBlank(allergy.getNarrativeText(), "Allergia o intolleranza")))
                    .append("</content>");

            if (allergy.getAgentCoding() != null) {
                xml.append("<br/>");
                xml.append("<content ID=\"").append(allergyId).append("-AGENT\">")
                        .append(t(defaultIfBlank(
                                allergy.getAgentCoding().getDisplayName(),
                                allergy.getAgentCoding().getCode()
                        )))
                        .append("</content>");
            }
            ...
            if (allergy.getReactionCoding() != null) {
                xml.append("<br/>");
                xml.append("<content ID=\"").append(allergyId).append("-REACTION\">")
                        .append(t(defaultIfBlank(
                                allergy.getReactionCoding().getDisplayName(),
                                allergy.getReactionCoding().getCode()
                        )))
                        .append("</content>");
            }
            ...
            if (allergy.getCriticalityCoding() != null) {
                xml.append("<br/>");
                xml.append("<content ID=\"").append(allergyId).append("-CRIT\">")
                        .append(t(defaultIfBlank(
                                allergy.getCriticalityCoding().getDisplayName(),
                                allergy.getCriticalityCoding().getCode()
                        )))
                        .append("</content>");
            }
            ...
            if (allergy.getStatusCoding() != null) {
                xml.append("<br/>");
                xml.append("<content ID=\"").append(allergyId).append("-STATUS\">")
                        .append(t(defaultIfBlank(
                                allergy.getStatusCoding().getDisplayName(),
                                allergy.getStatusCoding().getCode()
                        )))
                        .append("</content>");
            }
            ...
            if (hasText(allergy.getComment())) {
                xml.append("<br/>");
                xml.append("<content ID=\"").append(allergyId).append("-COMMENT\">")
                        .append(t(allergy.getComment()))
                        .append("</content>");
            }
        ...
        int index = 1;
        for (RsaDocument.Allergy allergy : valid) {
            String allergyId = "ALL-" + index;

            String actLow = resolveRequiredAllergyActLow(allergy, document, documentEffectiveTime);
            String actHigh = defaultIfBlank(allergy.getActEndDateTime(), allergy.getEndDateTime());

            String observationLow = resolveRequiredAllergyObservationLow(allergy, document, documentEffectiveTime);
            String observationHigh = allergy.getEndDateTime();

            String reactionLow = resolveRequiredAllergyReactionLow(allergy, document, documentEffectiveTime);
            String reactionHigh = allergy.getEndDateTime();

            boolean actCompleted = hasText(actHigh);

            xml.append("<entry>");
            xml.append("<act classCode=\"ACT\" moodCode=\"EVN\">");
            xml.append("<code nullFlavor=\"NA\"/>");

            xml.append("<statusCode code=\"")
                    .append(actCompleted ? "completed" : "active")
                    .append("\"/>");

            xml.append("<effectiveTime>");
            xml.append("<low value=\"").append(a(ts(actLow))).append("\"/>");
            if (hasText(actHigh)) {
                xml.append("<high value=\"").append(a(ts(actHigh))).append("\"/>");
            }
            xml.append("</effectiveTime>");
            ...
            xml.append("<text><reference value=\"#").append(allergyId).append("\"/></text>");
            xml.append("<statusCode code=\"completed\"/>");

            xml.append("<effectiveTime>");
            xml.append("<low value=\"").append(a(ts(observationLow))).append("\"/>");
            if (hasText(observationHigh)) {
                xml.append("<high value=\"").append(a(ts(observationHigh))).append("\"/>");
            }
            xml.append("</effectiveTime>");
            ...
            if (allergy.getAgentCoding() != null && hasAnyCodingValue(allergy.getAgentCoding())) {
                xml.append("<code");
                appendCodingAttributes(xml, allergy.getAgentCoding());
                xml.append(">");
                xml.append("<originalText><reference value=\"#").append(allergyId).append("-AGENT\"/></originalText>");
                xml.append("</code>");
            } else {
                xml.append("<code nullFlavor=\"UNK\"/>");
            }
            ...
            if (allergy.getReactionCoding() != null) {
                ...
                xml.append("<text><reference value=\"#").append(allergyId).append("-REACTION\"/></text>");
                xml.append("<statusCode code=\"completed\"/>");

                xml.append("<effectiveTime>");
                xml.append("<low value=\"").append(a(ts(reactionLow))).append("\"/>");
                if (hasText(reactionHigh)) {
                    xml.append("<high value=\"").append(a(ts(reactionHigh))).append("\"/>");
                }
                xml.append("</effectiveTime>");
                ...
                xml.append(">");
                xml.append("<originalText><reference value=\"#").append(allergyId).append("-REACTION\"/></originalText>");
                xml.append("</value>");
            ...
            if (allergy.getCriticalityCoding() != null) {
                ...
                xml.append("<text><reference value=\"#").append(allergyId).append("-CRIT\"/></text>");
                xml.append("<statusCode code=\"completed\"/>");
                xml.append("<value xsi:type=\"CD\"");
            ...
            if (allergy.getStatusCoding() != null) {
                ...
                xml.append("<observation classCode=\"OBS\" moodCode=\"EVN\">");
                xml.append("<code code=\"33999-4\" codeSystem=\"2.16.840.1.113883.6.1\" codeSystemName=\"LOINC\" displayName=\"Stato\"/>");
                xml.append("<text><reference value=\"#").append(allergyId).append("-STATUS\"/></text>");
                xml.append("<statusCode code=\"completed\"/>");
                xml.append("<value xsi:type=\"CD\"");
            ...
            if (hasText(allergy.getComment())) {
                ...
                xml.append("<text><reference value=\"#").append(allergyId).append("-COMMENT\"/></text>");
                xml.append("<statusCode code=\"completed\"/>");
                xml.append("</act>");
                xml.append("</entryRelationship>");
            }
    ...
    private void appendPrestazioni(StringBuilder xml, RsaDocument document) {
        List<RsaDocument.Prestazione> raw = safeClinical(document).getPrestazioni();
        List<RsaDocument.Prestazione> narrative = narrativePrestazioni(raw);
        ...
        int pIndex = 1;
        for (RsaDocument.Prestazione prestazione : narrative) {
            ...
            xml.append("<tr><th>Descrizione Prestazione eseguita</th><td>")
                    .append(t(defaultIfBlank(
                            prestazione.getText(),
                            prestazione.getCoding() != null ? prestazione.getCoding().getDisplayName() : ""
                    )))
                    .append("</td></tr>");
            ...
            if (!procedure.isEmpty()) {
                ...
                for (RsaDocument.ProceduraOperativa procedura : procedure) {
                    ...
                    xml.append("<table border=\"1\"><thead><tr><th colspan=\"2\">Procedura ")
                            .append(procIndex)
                    ...
                    xml.append("<tr><th>Descrizione Procedura Operativa</th><td>")
                            .append(t(defaultIfBlank(
                                    procedura.getText(),
                                    procedura.getCoding() != null ? procedura.getCoding().getDisplayName() : ""
                            )))
                            .append("</td></tr>");
        ...
        for (RsaDocument.Prestazione prestazione : narrative) {
             ...
             *
             * - coding == null:
             *   genera solo il blocco narrativo <text>, ma NON genera <entry>.
             *   Serve per CT17: blocco strutturato non presente.
             *
             * - coding != null ma vuoto/incompleto:
             *   genera <entry><act> ma può omettere <code>.
             *   Serve per i KO su codice prestazione mancante.
             */
            ...
            if (hasAnyCodingValue(prestazione.getCoding())) {
                xml.append("<code");
            ...
            for (RsaDocument.ProceduraOperativa procedura : validProcedure(prestazione.getProcedureOperative())) {
                ...
                if (procedura.getCoding() != null && hasAnyCodingValue(procedura.getCoding())) {
                    xml.append("<code");
    ...
    private List<RsaDocument.Prestazione> narrativePrestazioni(List<RsaDocument.Prestazione> prestazioni) {
        ...
        for (RsaDocument.Prestazione prestazione : prestazioni) {
            ...
            if (hasNarrativePrestazioneValue(prestazione)) {
                result.add(prestazione);
            }
        }
    ...
    private boolean hasNarrativePrestazioneValue(RsaDocument.Prestazione prestazione) {
        return hasText(prestazione.getText())
                || hasText(prestazione.getPerformedAt())
                || prestazione.getCoding() != null
                || !validProcedure(prestazione.getProcedureOperative()).isEmpty();
    }
    ...
    private void appendAdministrativeGenderCode(StringBuilder xml, String genderCode) {
        if (!hasText(genderCode)) {
            return;
        }
        xml.append("<administrativeGenderCode");
        appendAttributeIfPresent(xml, "code", genderCode);
        appendAttributeIfPresent(xml, "codeSystem", "2.16.840.1.113883.5.1");
        appendAttributeIfPresent(xml, "codeSystemName", "HL7 AdministrativeGender");
    ...
    private boolean hasAnyCodingValue(RsaDocument.Coding coding) {
        return coding != null && (
                hasText(coding.getCode())
                        || hasText(coding.getCodeSystem())
                        || hasText(coding.getCodeSystemName())
                        || hasText(coding.getDisplayName())
        );
    }
    ...
    private String resolveRequiredAllergyActLow(
            RsaDocument.Allergy allergy,
            RsaDocument document,
            String documentEffectiveTime
    ) {
        String value = firstNonBlank(
                allergy.getActStartDateTime(),
                allergy.getStartDateTime(),
                allergy.getActEndDateTime(),
                allergy.getEndDateTime(),
                resolveEncounterStartDateTime(document),
                resolveServiceEventEffectiveTime(document),
                documentEffectiveTime
        );

        if (!hasText(value)) {
            throw new IllegalStateException(
                    "Impossibile generare CDA RSA: Allergie act/effectiveTime/low obbligatorio non valorizzabile"
            );
        }
    ...
    private String resolveRequiredAllergyObservationLow(
            RsaDocument.Allergy allergy,
            RsaDocument document,
            String documentEffectiveTime
    ) {
        ...
        if (!hasText(value)) {
            throw new IllegalStateException(
                    "Impossibile generare CDA RSA: Allergie observation/effectiveTime/low obbligatorio non valorizzabile"
            );
        }
    ...
    private String resolveRequiredAllergyReactionLow(
            RsaDocument.Allergy allergy,
            RsaDocument document,
            String documentEffectiveTime
    ) {
        ...
        if (!hasText(value)) {
            throw new IllegalStateException(
                    "Impossibile generare CDA RSA: Allergie reaction/effectiveTime/low obbligatorio non valorizzabile"
            );
        }
    ...
    private String resolveEncounterStartDateTime(RsaDocument document) {
        return document != null && document.getEncounter() != null
                ? document.getEncounter().getStartDateTime()
                : null;
    }

    private String resolveServiceEventEffectiveTime(RsaDocument document) {
        return document != null && document.getServiceEvent() != null
                ? document.getServiceEvent().getEffectiveTime()
                : null;
    }
