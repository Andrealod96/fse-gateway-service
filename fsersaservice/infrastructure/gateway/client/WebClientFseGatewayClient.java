...
public class WebClientFseGatewayClient implements FseGatewayClient {
    ...
    /**
     * Recupero stato per workflowInstanceId.
     *
     * Nota importante:
     * il Gateway Docker/Sogei, per questo endpoint, può richiedere anche il secondo token
     * FSE-JWT-Signature. Per questo metodo NON usiamo il solo bearer, ma generiamo
     * entrambi gli header partendo dal CF paziente recuperato dal workflow locale.
     */
    @Override
    public Mono<GatewayTransactionInspectResponseDto> getStatusByWorkflowInstanceId(
        ...
        if (!StringUtils.hasText(patientFiscalCode)) {
            return Mono.error(new GatewayResponseParsingException(
                    "patientFiscalCode mancante per interrogazione stato workflow",
                    null,
                    null
            ));
        }

        return jwtGenerator.generateGatewayHeadersForWorkflowStatus(patientFiscalCode.trim())
    ...
    /**
     * Recupero stato per traceId.
     *
     * Nel documento ministeriale e nel tuo test locale questo endpoint funziona con
     * Authorization Bearer. Non aggiungiamo FSE-JWT-Signature qui finché non serve davvero.
     */
    @Override
    ...
    private Mono<GatewayValidationResponseDto> doValidateDocument(
        ...
        validateGatewayJwtHeaders(jwtHeaders, "validazione documento");
        ...
        return fseGatewayWebClient.post()
                ...
                .headers(headers -> {
                    ...
                    headers.set(HEADER_FSE_JWT_SIGNATURE, jwtHeaders.fseJwtSignatureToken());
                })
    ...
    private Mono<GatewayTransactionInspectResponseDto> doGetStatusByWorkflowInstanceId(
        ...
        log.info(
                "Chiamata Gateway workflow status preparata: path={}, workflowInstanceIdLength={}, authorizationPresent={}, fseJwtSignaturePresent={}",
                gatewayProperties.getStatusByWorkflowPath(),
                workflowInstanceId.length(),
                StringUtils.hasText(jwtHeaders.authorizationBearerToken()),
                StringUtils.hasText(jwtHeaders.fseJwtSignatureToken())
        );
    ...
    private Mono<GatewayTransactionInspectResponseDto> doGetStatusByTraceId(
        ...
        if (!StringUtils.hasText(authenticationBearerToken)) {
            return Mono.error(new GatewayResponseParsingException(
                    "Authorization Bearer JWT assente per interrogazione stato traceId",
                    null,
                    null
            ));
        }
        ...
        log.info(
                "Chiamata Gateway trace status preparata: path={}, traceId={}, authorizationPresent={}",
        ...
        return fseGatewayWebClient.get()
                ...
                .accept(MediaType.APPLICATION_JSON)
                .headers(headers -> headers.setBearerAuth(authenticationBearerToken))
                .exchangeToMono(response -> handleTypedResponse(
                        response,
                        gatewayProperties.getStatusByTracePath(),
                        this::parseTransactionInspectSuccessResponse
    ...
    private GatewayValidationResponseDto parseValidationSuccessResponse(
        ...
        GatewayValidationResponseDto dto =
                parseRequiredBody(httpStatus, rawBody, GatewayValidationResponseDto.class);
    ...
    private void validateGatewayJwtHeaders(GatewayJwtHeaders jwtHeaders, String operation) {
        if (jwtHeaders == null) {
            throw new GatewayResponseParsingException(
                    "JWT Gateway assenti per operazione: " + operation,
                    null,
                    null
            );
        }

        if (!StringUtils.hasText(jwtHeaders.authorizationBearerToken())) {
            throw new GatewayResponseParsingException(
                    "Authorization Bearer JWT assente per operazione: " + operation,
                    null,
                    null
            );
        }

        if (!StringUtils.hasText(jwtHeaders.fseJwtSignatureToken())) {
            throw new GatewayResponseParsingException(
                    "FSE-JWT-Signature assente per operazione: " + operation,
                    null,
                    null
            );
        }
