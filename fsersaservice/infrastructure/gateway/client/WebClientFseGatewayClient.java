...
public class WebClientFseGatewayClient implements FseGatewayClient {
    ...
    private Mono<GatewayValidationResponseDto> handleResponse(ClientResponse response) {
        ...
        return response.bodyToMono(String.class)
                ...
                .flatMap(rawBody -> {
                    if (httpStatus.is2xxSuccessful()) {
                        return Mono.fromCallable(() -> parseSuccessResponse(httpStatus, rawBody));
                    }
                    return Mono.error(toGatewayClientException(httpStatus, rawBody));
                });
    }

    private GatewayValidationResponseDto parseSuccessResponse(HttpStatusCode httpStatus, String rawBody) {
        ...
        final GatewayValidationResponseDto dto;
        try {
            dto = objectMapper.readValue(rawBody, GatewayValidationResponseDto.class);
        } catch (Exception ex) {
            throw new GatewayResponseParsingException(
    ...
    private GatewayClientException toGatewayClientException(HttpStatusCode httpStatus, String rawBody) {
    ...
    private GatewayTransportException toGatewayTransportException(Throwable ex) {
        String message = "Errore di trasporto verso Gateway FSE 2.0 su "
                + gatewayProperties.getValidationPath()
                + ": "
                + ex.getMessage();
