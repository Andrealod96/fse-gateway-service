...
public class JwtGenerator {
    ...
    private GatewayJwtHeaders generateGatewayHeadersBlocking(GatewayDocumentValidationRequest request) throws Exception {
        ...
        String patientCx = toPatientCx(request.getPatientFiscalCode());
    ...
    private GatewayJwtHeaders generateGatewayHeadersForWorkflowStatusBlocking(GatewayStatusJwtContext context) throws Exception {
        ...
        String patientCx = toPatientCx(context.patientFiscalCode());
        String issuerSuffix = resolveIssuerSuffix(keyMaterial.certificate());
    ...
    private String signSignatureTokenForDocumentValidation(
        ...
        boolean omitPurposeOfUse = isJwtNegativeTestMode(JwtNegativeTestMode.MISSING_PURPOSE_OF_USE);

        String effectiveActionId = isJwtNegativeTestMode(JwtNegativeTestMode.INVALID_ACTION_ID)
                ? "TEST"
                : request.getActionId();

        if (omitPurposeOfUse) {
            log.warn("Modalità accreditamento attiva: claim purpose_of_use omesso dal FSE-JWT-Signature");
        }

        if (isJwtNegativeTestMode(JwtNegativeTestMode.INVALID_ACTION_ID)) {
            log.warn("Modalità accreditamento attiva: claim action_id valorizzato con TEST");
        }
        ...
        if (!omitPurposeOfUse) {
            builder.claim("purpose_of_use", purposeOfUse);
        }
        ...
        builder
                ...
                .claim("action_id", effectiveActionId)
    ...
    private String toCx(String fiscalCode) {
        return toCx(fiscalCode, true);
    }

    private String toPatientCx(String fiscalCode) {
        return toCx(fiscalCode, !shouldPreserveRawPatientFiscalCodeInJwt());
    }

    private String toCx(String fiscalCode, boolean upperCase) {
        if (!StringUtils.hasText(fiscalCode)) {
            throw new JwtGenerationException("Codice fiscale mancante per costruire il claim CX");
        }

        String normalized = fiscalCode.trim();
        if (upperCase) {
            normalized = normalized.toUpperCase(Locale.ITALY);
        }
    ...
    private boolean shouldPreserveRawPatientFiscalCodeInJwt() {
        return accreditationGatewayTestProperties != null
                && accreditationGatewayTestProperties.isEnabled()
                && accreditationGatewayTestProperties.isPreserveRawPatientFiscalCodeInJwt();
    }
    ...
    private boolean isJwtNegativeTestMode(JwtNegativeTestMode mode) {
        return accreditationGatewayTestProperties != null
                && accreditationGatewayTestProperties.isEnabled()
                && accreditationGatewayTestProperties.getJwtNegativeTestMode() == mode;
    }
