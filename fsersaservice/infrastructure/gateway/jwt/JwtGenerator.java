...
public class JwtGenerator {
    ...
    private GatewayJwtHeaders generateGatewayHeadersBlocking(GatewayDocumentValidationRequest request) throws Exception {
        ...
        String subjectCx = toCx(request.getPractitionerFiscalCode());
        String patientCx = toCx(request.getPatientFiscalCode());

        // FIX: usa issuerIdentifier dalla config se valorizzato, altrimenti CN del certificato.
        // In ambiente Docker di test impostare fse.jwt.issuer-identifier=<codiceStruttura>.
        ...
        String authToken = signAuthToken(request, keyMaterial, issuerSuffix, subjectCx, now, exp);
     ...
     * Priorità: 1) fse.jwt.issuer-identifier dalla configurazione, 2) CN del certificato X.509.
     */
    ...
    private String signSignatureToken(
        ...
        if (StringUtils.hasText(locality)) {
            builder.claim("locality", locality);
        }
